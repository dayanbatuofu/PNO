...
model = modules_hno.SingleBVPNet(in_features=5, out_features=64, type=opt.model, mode=opt.mode,
                                 final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)

model.to(device)

loss_fn_supervised = loss_functions.initialize_intersection_HJI_supervised(hybrid_dataset, Weight1, alpha)
loss_fn_hybrid = loss_functions.initialize_intersection_HJI_hyrid(hybrid_dataset, Weight2, alpha)
