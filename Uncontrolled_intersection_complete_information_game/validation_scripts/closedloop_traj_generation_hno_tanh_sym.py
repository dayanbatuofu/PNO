...
def discrete_data(data, dt, theta, N):
    ...
    for i in range(len(U1[:, 0])):
        for j in range(time_horizon):
            ...
            x1_in1 = (x1 - R1 / 2 + theta1 * W2 / 2) * 5
            x1_out1 = -(x1 - R1 / 2 - W2 / 2 - L1) * 5
            x2_in1 = (x2 - R2 / 2 + theta2 * W1 / 2) * 5
            x2_out1 = -(x2 - R2 / 2 - W1 / 2 - L2) * 5
            x1_in2 = (x1 - R1 / 2 + theta1 * W2 / 2) * 5
            x1_out2 = -(x1 - R1 / 2 - W2 / 2 - L1) * 5
            x2_in2 = (x2 - R2 / 2 + theta2 * W1 / 2) * 5
            x2_out2 = -(x2 - R2 / 2 - W1 / 2 - L2) * 5
            Loss1_tmp[i][j] = (U1[i][j] ** 2 + beta * torch.sigmoid(x1_in1) * torch.sigmoid(x1_out1) * torch.sigmoid(
                x2_in1) * torch.sigmoid(x2_out1)) * t_step
            Loss2_tmp[i][j] = (U2[i][j] ** 2 + beta * torch.sigmoid(x1_in2) * torch.sigmoid(x1_out2) * torch.sigmoid(
                x2_in2) * torch.sigmoid(x2_out2)) * t_step
...
if __name__ == '__main__':
    ...
    alpha = 10
    theta = (int(policy1[N_choice][0]), int(policy1[N_choice][2:]))

    ckpt_path = './model/tanh/model_hno.pth'
    activation = 'tanh'

    # Initialize and load the model
    model = modules_hno.SingleBVPNet(in_features=5, out_features=64, type=activation, mode='mlp',
    ...
    # t = np.repeat(np.flip(Time).reshape(1, -1), 2601, axis=0).reshape(1, -1)
    ...
    if save_data:
        save_path = 'closed_loop/tanh/closedloop_traj_hno_initial_' + str(policy1[N_choice]) + '_tanh.mat'
        # save_path = 'closed_loop/tanh/closedloop_traj_hno_initial_' + str(policy1[N_choice]) + '_tanh_nc.mat'
